...
<head>
    ...
    <!-- Toastr JS -->
    <title>Forget Password Reset</title>
    <style>
        <style>
        /* default css */
...
<body style="display:flex; align-items:center; justify-content:center;">
    <div class="login-page">
        @if (session('message'))
            ...
            //         }
            //     }, 60,000); // 3000ms = 3 seconds
            // </script>
        @endif


        <div class="form">
            <form id="resetPasswordForm" class="login-form" action="{{ route('forget_password_reset_store') }}"
                method="POST">
                @csrf
                <h5 style=""></i>Forget Password Reset</h2>

                    <input class="mb-1" type="text" name="otp" class="form-control" min="11"
                        placeholder="Enter OTP Number" required>


                    <input class="mb-1" type="password" name="password" id="password" class="form-control"
                        min="6" placeholder="Enter New Password" required>

                    <input class="mb-1" type="password" name="password_confirmation" id="password_confirmation"
                        class="form-control" min="6" placeholder="Enter Confirm Password" required>
                    @if (session('message'))
                    <a style="text-decoration: none;" class="sign-up"
                            href="{{ route('forget_password_otp') }}">Resend OTP</a>
                    @else
        ...
        $(document).ready(function() {
            $('#resetPasswordForm').on('submit', function(e) {
                ...
                if (password.length < 6) {
                    $('#password').after(
                        '<span class="form-error" style="color:red;">Password must be at least 6 characters long.</span>'
                        );
                    isValid = false;
                }
                ...
                if (password !== confirmPassword) {
                    $('#password_confirmation').after(
                        '<span class="form-error" style="color:red;">You Confimation Password do not match.</span>'
                        );
                    isValid = false;
                }

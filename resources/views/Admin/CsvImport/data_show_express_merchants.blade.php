    ...
    <body style="background-color:#f5f5f5;">
                                ...
                                <tr>
                                    ...
                                    <td class="edit_ignore">
                                        <select class="form-control zone" data-id="{{ $j }}"
                                            ...
                                            @endforeach --}}
                                            @foreach ($district as $zone_data)
                                                <option value="{{ $zone_data->d_name }}">{{ $zone_data->d_name }}
                                                </option>
                                            @endforeach
                                        </select>
                                    </td>
        ...
        <script>
            ...
            $('#csv_table tbody').on('change', '.zone', function() {
                ...
                $.ajax({
                    url: "{{ route('ajaxdata.area.merchants') }}",
                    type: "POST",
                    data: {
                        _token: '{{ csrf_token() }}',
                        zone: zone
            ...
            $('.Submitdata').on('click', function() {
                ...
                $.ajax({
                    url: "{{ route('file-data-submit-express-merchant') }}",
                    type: "POST",
                    data: {
                        "_token": "{{ csrf_token() }}",
                        'data': data,

    ...
    */
    ...
    public function rider_status_date(Request $request)
    {
        ...
        if ($status === 'Successfully Delivered') {
            $order1 = OrderStatusHistory::where('order_status_histories.user_id', $rider)
                ->whereDate('order_status_histories.created_at', '>=', $fromdate)
                ->whereDate('order_status_histories.created_at', '<=', $todate)
            ...
            $total_commission_rider = Rider::where('user_id', $request->rider)->first();

            $rider_commission = $Qty1 * $total_commission_rider->r_delivery_charge;
        } else {
            $rider_commission = 0;
        }
        ...
        return view(
            ...
            compact('fromdate', 'rider_commission', 'todate', 'user', 'rider', 'status', 'order', 'Qty', 'filtering')
        );
    }
    ...
    public function rider_status_date_print(Request $request)
    {
        ...
        if ($status === 'Successfully Delivered') {
            $orders1 = OrderStatusHistory::where('order_status_histories.user_id', $request->rider)
                ->where('order_status_histories.status', $status)
                // ->whereBetween('order_status_histories.updated_at', [$fromdate, $todate])
                ->whereDate('order_status_histories.updated_at', '>=', $fromdate)
                ->whereDate('order_status_histories.updated_at', '<=', $todate)
        ...
        return view(
            ...
            compact('fromdate', 'todate', 'rider_commission', 'rider', 'status', 'orders', 'Qty', 'company')
        );
    }

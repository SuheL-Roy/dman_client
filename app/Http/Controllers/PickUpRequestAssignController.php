...
class PickUpRequestAssignController extends Controller
{
    ...
    public function request_pickup()
    {
        if (auth()->user()->role == 8) {
            ...
            $order = Order::orderBy('order_confirms.id', 'DESC')
                ...
                ->select('orders.*', 'order_confirms.*', 'merchants.*', 'users.*', 'agents.*')
    ...
    public function list_auto(Request $request)
    {
        ...
        if ($request->merchant_id) {
            ...
            $datas = AutoAssign::orderBy('orders.id', 'DESC')
        ...
        return view('Admin.PickUpRequestAssign.assignedRequest_auto', compact('datas', 'selectedMerchant', 'merchants'));
    }
    ...
    public function collect_auto(Request $request)
    {
        ...
        return redirect()->route('request.assign.auto.lists')->with('message', 'Order Collect Successfully');
    }
    public function collectAll_auto(Request $request)
    {
        ...
        foreach ($trackings as $tracking) {
            ...
            // $riderInfo = User::where('id', Auth::user()->id)->first();
            $company = Company::where('id', 1)->first();
    ...
    public function collect(Request $request)
    {
        ...
        $text = "Dear Valued Customer,\n{$company->name} Received a parcel From \"{$data->business_name}\" Value - {$data->collection} TK and It will be delivered Soon.\nThanks \n{$company->website}/live-tracking-parcel?tracking_id={$request->id}";
    ...
    public function deleteOrderAdmin(Request $request)
    {
        ...
        if ($request->id) {
            ...
            \Toastr::success('Status cancelled Successfully!', 'Success!!!', ["positionClass" => "toast-bottom-right", "progressBar" => true]);
    ...
    public function admin_order_delete(Request $request)
    {
        // return $request->id;
        // return $request->all();
        // $data = new OrderStatusHistory();
        ...
        if ($request->id) {
            ...
            \Toastr::success('Order Data Deleted Successfully!', 'Success!!!', ["positionClass" => "toast-bottom-right", "progressBar" => true]);
    ...
    public function rider_payment(Request $request)
    {
        ...
        $order = OrderStatusHistory::where('order_status_histories.user_id', $rider)
            ...
            ->whereIn('order_status_histories.status', ['Successfully Delivered', 'Pickup Done'])
            ->join('orders', 'orders.tracking_id', 'order_status_histories.tracking_id')
            ->join('order_confirms', 'orders.tracking_id', 'order_confirms.tracking_id')
            ->join('merchants', 'orders.user_id', 'merchants.user_id')
            ->join('users', 'order_status_histories.user_id', 'users.id')
            ->join('riders', 'order_status_histories.user_id', 'riders.user_id')
            ->select('orders.tracking_id', 'order_status_histories.created_at', 'users.name', 'users.mobile', 'users.address', 'riders.r_delivery_charge', 'riders.r_pickup_charge', 'riders.r_return_charge', 'merchants.business_name', 'orders.customer_name', 'orders.customer_phone', 'orders.customer_address', 'orders.delivery_note', 'order_status_histories.status', 'order_confirms.colection', 'order_confirms.collect')
            ->get();

        $success_order = OrderStatusHistory::where('order_status_histories.user_id', $rider)
            // ->whereBetween('order_status_histories.updated_at', [$fromdate, $todate])
            ->whereDate('order_status_histories.created_at', '>=', $fromdate)
            ->whereDate('order_status_histories.created_at', '<=', $todate)
        ...
        $pick_up_order = OrderStatusHistory::where('order_status_histories.user_id', $rider)
            // ->whereBetween('order_status_histories.updated_at', [$fromdate, $todate])
            ->whereDate('order_status_histories.created_at', '>=', $fromdate)
            ->whereDate('order_status_histories.created_at', '<=', $todate)
        ...
        $total_delivery_charge = $success_order->sum('r_delivery_charge');
        $total_pick_charge = $pick_up_order->sum('r_pickup_charge');
        $total_return_charge = $order->sum('r_return_charge');

        $subTotal = $total_delivery_charge + $total_pick_charge + $total_return_charge;


        return view('Admin.PaymentInfo.rider_payment', compact('user', 'subTotal', 'rider', 'total_delivery_charge', 'total_pick_charge', 'total_return_charge', 'fromdate', 'todate', 'order'));
    }
    ...
    public function rider_payment_store(Request $request)
    {
        ...
        $orders = OrderStatusHistory::where('order_status_histories.user_id', $rider)
            // ->whereBetween('order_status_histories.updated_at', [$fromdate, $todate])
            ->whereDate('order_status_histories.created_at', '>=', $fromdate)
            ->whereDate('order_status_histories.created_at', '<=', $todate)
            ->whereIn('order_status_histories.status', ['Successfully Delivered', 'Pickup Done'])
            ...
            ->select('orders.tracking_id', 'order_status_histories.created_at', 'users.name', 'users.mobile', 'users.address', 'riders.r_delivery_charge', 'riders.r_pickup_charge', 'riders.r_return_charge', 'merchants.business_name', 'orders.customer_name', 'orders.customer_phone', 'orders.customer_address', 'orders.delivery_note', 'order_status_histories.status', 'order_confirms.colection', 'order_confirms.collect')
            ->get();
        $total_return_charge = $orders->sum('r_return_charge');



        $invs = "RPMH";
        $riderLastId = RiderPaymentHistory::all()->count() + 1;
        $auth_id = Auth::user()->id;
        $rand_number = rand(111, 999);
        $invoice = $invs . $auth_id . $riderLastId . $rand_number;
        ...
        if ($orders) {
            ...
            $data = new RiderPaymentHistory();
            $data->invoice_id = $invoice;
            $data->form_date = $fromdate;
            $data->todate = $todate;
            $data->rider_id = $rider;
            $data->t_pickup_commission_charge = $total_pick_charge;
            $data->t_delivery_charge = $total_delivery_charge;
            $data->t_return_commission_charge     = $total_return_charge;
            $data->Total     = $total_pick_charge + $total_delivery_charge + $total_return_charge;
            $data->status = 1;
            ...
            foreach ($orders as $order) {
                ...
                $data = new RiderPaymentHistoryDetails();
                $data->invoice_id  = $invoice;
                $data->rider_id = $rider;
                $data->tracking_id = $order->tracking_id;
    ...
    public function branch_payment(Request $request)
    {
        // dd($request->all());
        $today = date('Y-m-d');
        ...
        if ($agentInfo) {
            $order = OrderStatusHistory::orderBY('order_status_histories.id', 'DESC')
                ->whereDate('order_status_histories.created_at', '>=', $fromdate)
                ->whereDate('order_status_histories.created_at', '<=', $todate)
                ->whereIn('order_status_histories.status', ['Successfully Delivered', 'Pickup Done'])
                ->join('riders', 'order_status_histories.user_id', 'riders.user_id')
                ->where('riders.area', $agentInfo->area)
                ->join('users', 'order_status_histories.user_id', 'users.id')
                ->join('orders', 'orders.tracking_id', 'order_status_histories.tracking_id')
                ->join('order_confirms', 'orders.tracking_id', 'order_confirms.tracking_id')
                ->join('merchants', 'orders.user_id', 'merchants.user_id')
                ->join('agents', 'agents.area', 'merchants.area')
                ->select('users.name', 'agents.area', 'agents.a_delivery_charge', 'agents.a_pickup_charge', 'agents.a_return_charge', 'order_status_histories.created_at as create_date', 'order_status_histories.tracking_id', 'order_status_histories.status', 'merchants.business_name')
                ->get();

            $delivery = OrderStatusHistory::orderBY('order_status_histories.id', 'DESC')
                ->whereDate('order_status_histories.created_at', '>=', $fromdate)
                ->whereDate('order_status_histories.created_at', '<=', $todate)
                ->where('order_status_histories.status', 'Successfully Delivered')
                ->join('riders', 'order_status_histories.user_id', 'riders.user_id')
            ...
            $pickup = OrderStatusHistory::orderBY('order_status_histories.id', 'DESC')
                ->whereDate('order_status_histories.created_at', '>=', $fromdate)
                ->whereDate('order_status_histories.created_at', '<=', $todate)
                ->where('order_status_histories.status', 'Pickup Done')
                ->join('riders', 'order_status_histories.user_id', 'riders.user_id')
            ...
            $total_delivery_charge = $delivery->sum('a_delivery_charge');
            $total_pick_charge = $pickup->sum('a_pickup_charge');
            $total_return_charge = $order->sum('a_return_charge');
            $total = $total_delivery_charge + $total_pick_charge + $total_return_charge;

            return view('Admin.PaymentInfo.branch_payment', compact('user', 'total', 'total_delivery_charge', 'total_pick_charge', 'total_return_charge', 'agent', 'fromdate', 'todate', 'order'));
        }




        return view('Admin.PaymentInfo.branch_payment', compact('user', 'agent', 'fromdate', 'todate'));
    }

    public function branch_payment_store(Request $request)
    {
        ...
        $total_delivery_charge = $delivery->sum('a_delivery_charge');
        $total_pick_charge = $pickup->sum('a_pickup_charge');
        $total_return_charge = $orders->sum('a_return_charge');

        $invs = "MPH";
        $riderLastId = AgentPaymentHistory::all()->count() + 1;
        $auth_id = Auth::user()->id;
        $rand_number = rand(111, 999);
        $invoice = $invs . $auth_id . $riderLastId . $rand_number;
        ...
        if ($orders) {
            ...
            $data->save();
            foreach ($orders as $order) {
                $data = new AgentPaymentHistoryDetails();
                $data->invoice_id  = $invoice;
                $data->agent_id     = $agent;
                $data->tracking_id = $order->tracking_id;
                $data->status = $order->status;
